      ...
      <View style={[styles.courtOptionCard, { backgroundColor: colors.card }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, flex: 1 }}>
          ...
          <View style={styles.courtOptionInfo}>
            <Text style={[styles.courtOptionName, { color: colors.text }]}>{method.name}</Text>
            <Text style={[styles.courtOptionDesc, { color: colors.textSecondary }]}>{method.description}</Text>
          </View>
        </View>
        {selectedPayment === method.id && <Ionicons name="checkmark-circle" size={24} color={AppColors.primary} />}
      </View>
    </TouchableOpacity>
  );
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.courtSummary, { borderBottomColor: colors.border }]}>
          ...
          <View style={styles.courtSummaryInfo}>
            <Text style={[styles.courtSummaryName, { color: colors.text }]}>{bookingData.courtName}</Text>
            <View style={styles.courtSummaryMeta}>
              <Ionicons name="location-outline" size={16} color={colors.icon} />
              <Text style={[styles.courtSummaryMetaText, { color: colors.textSecondary }]}>{bookingData.location}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          >
            ...
            <View style={{ gap: 4 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '600', color: colors.text }}>{bookingData.court}</Text>
              <Text style={{ fontSize: 13, color: colors.textSecondary }}>{bookingData.courtDescription}</Text>
            </View>
            ...
            <View style={{ gap: 8 }}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                {bookingData.selectedTimes.map((time: string) => (
                  ...
                  >
                    <Text style={{ fontSize: 13, fontWeight: '500', color: colors.text }}>{time}</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          >
            {bookingData.selectedTimes.map((time: string, index: number) => (
              <View key={time} style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ fontSize: 14, color: colors.textSecondary }}>Giờ {time}</Text>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              <Text style={{ fontSize: 16, fontWeight: '600', color: colors.text }}>Tổng cộng</Text>
      ...
      <View style={[styles.footer, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
        <View style={styles.bookingSummary}>
          <View style={styles.summaryItem}>
            ...
            <Text style={styles.priceHighlight}>{bookingData.totalPrice.toLocaleString('vi-VN')}đ</Text>
          </View>
        </View>
        <TouchableOpacity style={styles.continueBtn} onPress={handleConfirmBooking}>
